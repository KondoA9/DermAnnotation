export default {
  en: {
    downloadPage: {
      date: "Updated: December 6, 2022",
      noteForMac:
        'For macOS, a warning will be displayed at the first boot after the update. You can start it normally by right-clicking while pressing the control key from Finder and selecting "Open".',
      releaseLabel: "Latest version update information",
      bugLabel: "Known bugs",
      releases: [
        "[Feature]: Added function to delete default pen setting of workspace and root folder in pen edit dialog",
        "[Other]: Minor usability improvements",
      ],
      bugs: [],
    },
  },
  ja: {
    downloadPage: {
      date: "最終更新: 2022/12/06",
      noteForMac:
        "macOSの場合、アップデート後の初回起動時に警告が表示されます。Finderからcontrolキーを押下しながら右クリックし、「開く」を選択することで正常に起動できます。",
      releaseLabel: "最新バージョンの更新情報",
      bugLabel: "既知の不具合",
      releases: [
        "[Feature]: ペン編集ダイアログにワークスペース及びルートフォルダのデフォルトのペン設定を削除する機能を追加",
        "[Other]: 細かい使いやすさの向上",
      ],
      bugs: [],
    },
  },
}
