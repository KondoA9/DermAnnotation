<template>
  <div>
    <Header />
    <div
      class="container-fluid py-5 text-center bg-light my-auto"
      style="min-height: 90vh"
    >
      <h2 class="display-5 pt-3">Download DermAnnotation v2.0.8</h2>
      <p class="my-3">{{ $t("downloadPage.date") }}</p>
      <div class="container" style="display: table">
        <div class="align-middle" style="display: table-cell">
          <div>
            <button
              class="btn btn-primary py-3 px-5 m-3"
          ...
          <div class="container pt-5 pb-3" style="text-align: left">
            <div v-show="$t('downloadPage.releases').length !== 0">
              ...
              <ul class="py-2">
                <li
                  v-for="release in $t('downloadPage.releases')"
                  :key="release"
                >
                  {{ release }}
                </li>
...
export default Vue.extend({
  ...
  methods: {
    download(platform: "windows" | "macOS") {
      const installer =
        platform === "windows"
          ? "DermAnnotationSetup_Windows.msi"
          : "DermAnnotationSetup_macOS.zip"
